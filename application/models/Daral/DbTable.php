...
{    
	...
	public function getDaralStats($numDaral)
	{
		$return = array();
	
	
	
		$nbFarmer  = "select count(*) from farmer where isactive_farmer = 1 and daral_actuel=".$numDaral;
		$nbCheptel = "select count(*) from animal a, farmer f where f.isactive_farmer = 1 and f.daral_actuel=".$numDaral." and f.id_farmer=a.fk_id_farmer and a.isactive= 1";
		
		$nbNotific = "select count(*) from notification n,localite l,daral d where d.name=".$numDaral." and d.id_localite=l.name and l.id=n.id_localite ";
		$nbNotifVol = "select count(*) from notification n,localite l,daral d ,typenotification t where d.name=".$numDaral." and d.id_localite=l.name and l.id=n.id_localite and n.type=t.id and t.libelle like '%vol%' ";
		/*
		$nbNotifVol="select count(*) from notification n,typenotification t where n.type=t.id and t.libelle like '%vol%'";
		$LastNotifVol="select n.description,n.id_localite from notification n order by n.id desc limit 1";*/
