 ...
 * @version $Id$
 *
 * @method Application_Model_Localite_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_Localite_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_Localite_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_Localite_Rowset find()
 *
 */
abstract class Application_Model_Localite_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
  ...
  3 => 'Application_Model_Notification_DbTable',
    ...
    protected $_referenceMap = array(        
        'region' => array(
            'columns' => 'region',
            'refTableClass' => 'Application_Model_Region_DbTable',
            'refColumns' => 'name'
        ),
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_Localite_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_Localite_Rowset';
  ...
  'region' => NULL,
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['departement']) && !empty($params['departement'])) {
            $select->where('departement = ?', $params['departement']);
        }

        if (isset($params['region']) && !empty($params['region'])) {
            $select->where('region = ?', $params['region']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'departement' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('departement LIKE ?', "%$keywords%");
            }

            if ('all' === $searchMode || 'region' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('region LIKE ?', "%$keywords%");
            }
