 ...
 * @version $Id$
 *
 * @method Application_Model_Maladie_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_Maladie_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_Maladie_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_Maladie_Rowset find()
 *
 */
abstract class Application_Model_Maladie_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
    protected $_name = 'maladie';
  ...
  0 => 'Application_Model_Media_DbTable',
);
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_Maladie_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_Maladie_Rowset';
  ...
  'vaccin' => NULL,
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['symptomes']) && !empty($params['symptomes'])) {
            $select->where('symptomes = ?', $params['symptomes']);
        }
        ...
        if (isset($params['vaccin']) && !empty($params['vaccin'])) {
            $select->where('vaccin = ?', $params['vaccin']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'symptomes' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('symptomes LIKE ?', "%$keywords%");
            }
            ...
            if ('all' === $searchMode || 'vaccin' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('vaccin LIKE ?', "%$keywords%");
            }
