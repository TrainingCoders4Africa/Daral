 ...
 * @version $Id$
 *
 * @method Application_Model_Media_Row createRow(array $data, string $defaultSource = null)
 * @method Application_Model_Media_Rowset fetchAll(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $count = null, int $offset = null)
 * @method Application_Model_Media_Row fetchRow(string|array|Zend_Db_Table_Select $where = null, string|array $order = null, int $offset = null)
 * @method Application_Model_Media_Rowset find()
 *
 */
abstract class Application_Model_Media_DbTable_Abstract extends Zend_Db_Table_Abstract
{
    /**
     * @var string
     */
  ...
  0 => 'id_media',
    ...
    protected $_referenceMap = array(        
        'maladie' => array(
            'columns' => 'maladie',
            'refTableClass' => 'Application_Model_Maladie_DbTable',
            'refColumns' => 'libelle'
        ),

        'langue' => array(
            'columns' => 'langue',
            'refTableClass' => 'Application_Model_Langue_DbTable',
            'refColumns' => 'libelle'
        )
    );
     ...
     * @var string
     */
    protected $_rowClass = 'Application_Model_Media_Row';

    /**
     * @var string
     */
    protected $_rowsetClass = 'Application_Model_Media_Rowset';
  ...
  'maladie' => NULL,
    ...
    public function getDbSelectByParams($params = array(), $sortField = '', $sortOrder = '')
    {
        ...
        if (isset($params['id_media']) && !empty($params['id_media'])) {
            $select->where('id_media = ?', $params['id_media']);
        }
        ...
        if (isset($params['langue']) && !empty($params['langue'])) {
            $select->where('langue = ?', $params['langue']);
        }

        if (isset($params['date_insertion']) && !empty($params['date_insertion'])) {
            $select->where('date_insertion = ?', $params['date_insertion']);
        }

        if (isset($params['lien']) && !empty($params['lien'])) {
            $select->where('lien = ?', $params['lien']);
        }

        if (isset($params['maladie']) && !empty($params['maladie'])) {
            $select->where('maladie = ?', $params['maladie']);
        }
        ...
        if (isset($params['_kw']) && !empty($params['_kw'])) {
            ...
            if ('all' === $searchMode || 'langue' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('langue LIKE ?', "%$keywords%");
            }

            if ('all' === $searchMode || 'lien' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('lien LIKE ?', "%$keywords%");
            }

            if ('all' === $searchMode || 'maladie' === $searchMode) {
                $searchWheres[] = $dbAdapter->quoteInto('maladie LIKE ?', "%$keywords%");
            }
